  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (!bodyData) {
        return res
          .status(400)
          .json({ error: "Missing reportData in request body" });
      }
      reportData =
    ...
    page.drawText(
      `Planting Date: ${reportData.plantingDate || "Not specified"}`,
      {
        x: margin,
        y,
        size: fontSize,
    ...
    if (reportData.coordinates && reportData.coordinates.length > 0) {
      ...
      if (reportData.coordinates[0] && reportData.coordinates[0].length > 0) {
        for (
          let i = 0;
          i < Math.min(maxCoords, reportData.coordinates[0].length);
          i++
        ) {
          if (y < margin + 50) {
          ...
          if (Array.isArray(coord) && coord.length >= 2) {
            const coordText = `Point ${i + 1}: Lat ${coord[1].toFixed(
              6
        ...
        if (reportData.coordinates[0].length > maxCoords) {
          ...
          page.drawText(
            `... and ${
              reportData.coordinates[0].length - maxCoords
            } more points`,
            {
              x: margin,
              y,
    ...
    if (
      ...
      Object.keys(reportData.weatherData).length > 0
    ) {
      y -= 40;
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename=${reportData.fieldName.replace(/\s+/g, "_")}_report.pdf`
    );
