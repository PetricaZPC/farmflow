...
function Profile({ userEmail }) {
  ...
  const handleFriendRequest = async (requestId, accepted) => {
    try {
      const requestToRespond = friendRequests.find(
        (req) =>
      ...
      if (accepted) {
        const newFriend = {
          ...
          username:
            requestToRespond.fromUsername ||
            requestToRespond.fromEmail.split("@")[0],
                ...
                <>
                  ...
                  <div className="space-y-3 mb-4">
                    {friendRequests.map((request) => (
                      <div
                        key={
                          request._id ? request._id.toString() : Math.random()
                        }
